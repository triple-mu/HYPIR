...
SIGMA_RANGE = (0.5, 2.4)      # v1 (1.85,3.45) -> v2 (0.6,6.2) -> 现在 (0.5,2.4)。
                              # 量纲此前一直没核对：_lowpass 里 f0=0.1325/sigma 用的是
                              # fftfreq 约定（奈奎斯特=0.5），而 MTF 归一化到奈奎斯特=1，
                              # 差一倍。1D MTF 实测真实 f0_MTF=0.19~0.33，换算 sigma=0.80~1.39，
                              # 旧上界 6.2 是它的四倍多——大量样本远重于真实。
ANISO_RANGE = (1.0, 2.0)      # v1 (1.0,1.45)
POLE_RANGE = (1.8, 4.5)       # v1 (0.9,2.1) -> v2 (0.7,3.0) -> 现在 (1.8,4.5)。
                              # 1D MTF 实测：真实退化拟合 1/(1+(f/f0)^2)^p 得 p=2.9~3.0，
                              # 顶到旧上界。低频落点 62-67 分位、中频 27-35 分位说明
...
SCALE_RANGE = (3.5, 10.0)     # 用 1D MTF 与真实曲线逐频段匹配标定：case1 7.5x /
                              # case2 5.0x / case3 8.0x，故区间围绕 5~8 展开留余量。
                              # 旧的 (1.6,7.0) 均匀采样期望 4.3x，过半样本比真实轻。
