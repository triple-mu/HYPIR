...
**host 侧的三个隐藏开销**（4 卡 H200 / batch 8，差分实测，非 tqdm 均值）

一开始步时 2.65 s，而文档记的 compile 后模型侧只有 630 ms —— 约 2 秒不在模型上。
GPU 利用率均值 28%、64.8% 的采样点低于 10%，四个 rank 却各自死占 1 个核，
而系统整体 CPU 才 2.3%：典型的 host 侧单线程串行阻塞。逐个拆掉：

| 改动 | 步时 | GPU 均值 |
|---|---:|---:|
| 起点 | 2.650 s | 28.0% |
| 训练循环里每步 15 次 GPU→CPU 硬同步 | 2.650 s | 18.9% |
| EMA 每步重建 514 次全参数字典 | 0.862 s | 59.0% |
| 判别器损失每次调用 4 次阻塞 H2D | **0.795 s** | **77.8%** |

- 同步那一步**没换来速度**，只是把遮挡物拿开让真瓶颈露出来。三个热点是串联的，
  必须按这个顺序拆，先量到哪个就先修哪个会得出错误结论。
- EMA 那个是自己人写的坑：`dict(self.model.named_parameters())` 写在了列表推导式内部。
  diffusers 的 `EMAModel.step(parameters)` 把参数列表当入参传进来，结构上就不会有这个
  问题；但它用按位置的 `shadow_params`，换过去要动 checkpoint 格式 / `eval_p0.py` /
  官方权重续训三处，而修好后 EMA 只占 0.1%，不值得。**accelerate 没有 EMA。**
- 判别器损失那个改写务必钉死 `dtype=torch.float32`：上游 `.to(device)` 只换设备不换
  dtype，BCE 一直在 fp32 下算，跟着输入变 fp16 会让损失差 2e-3。已做 15 组逐位对拍。

**还剩的**：`decode_jpeg` + 退化配方占 21%，现在是在训练循环里同步跑、占默认流、和计算
串行。要开侧流预取下一批，难点是退化的 RNG 得跟着错开一拍且各 rank 不能撞。未做。
...
| `FINDINGS-P3训练.md` | **提交包一直装的是未微调基线（19.0%）**，换 P2@7000 就是 45.6%。P2/P3 权重对比、P3 曲线、打包端到端验证、机器链路带宽 |
...
**评估刻度**：未微调 HYPIR **19.0%** → P2 5000 步 44.0% / 7000 步 **45.6%** → P3 8000 步 **45.4%**（裸权重，与 P2 打平且仍在爬，已续训到 16000）。
详见 `docs/FINDINGS-P3训练.md`，逐样本原始结果在 `docs/eval_p0/`。
